...
-export([start/0, stop/0, create_db/0, start_db/0, write/1, create_ride/1, create_rider/1,
         lookup/2, match/1, select/1]).
...
-spec create_ride(binary()) -> {ok, RideId} when RideId :: id().
create_ride(Name) ->
    RideId = rl_util:id(),
...
-spec create_rider(binary()) -> {ok, RiderId} when RiderId :: id().
create_rider(Name) ->
    RiderId = rl_util:id(),
    %% TODO: might need to wrap in a transaction
